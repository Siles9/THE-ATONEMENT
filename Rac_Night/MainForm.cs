...
namespace Rac_Night
{
    public partial class MainForm : Form
    {
        ...
        private const int LongPauseMs = 5500;  // Пауза между этапами
        ...
        public MainForm()
        {
            ...
            typingTimer.Interval = 70; // Скорость печати
        ...
        private async Task ProcessCutsceneStage()
        {
            ...
            switch (currentStage)
            {
                ...
                case CutsceneStage.Finished:
                    ...
                    // ОЧЕНЬ ВАЖНО: Останавливаем все ресурсы перед закрытием формы
                    StopAllResources();
                    this.Close(); // Теперь можно безопасно закрыть MainForm, т.к. Application.Run() не привязан к ней.
        ...
        // --- Новый метод для остановки и освобождения всех ресурсов MainForm ---
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            // Этот метод теперь будет вызван при this.Close() в ProcessCutsceneStage.
            // Также он будет вызван, если пользователь закроет приложение иначе (например, Alt+F4)
            StopAllResources(); // Вызываем тот же метод для освобождения ресурсов
        }
    }
}
